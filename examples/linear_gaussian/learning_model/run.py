...
class learn_linear_gaussian():
    def __init__(self, config_data, campaign_path, prior_mean, prior_cov):
        ...
        # Forward model
        self.linear_gaussian_model = linear_gaussian(spatial_res=self.spatial_resolution)
        self.vm = self.linear_gaussian_model.compute_vm()
        self.sub_sampled_vm = self.vm[self.sample_idx, :]

        self.sample_idx_mat = np.zeros((self.spatial_resolution, config_data["total_samples"]))
    ...
    def plot_mle_estimate(self, theta_mle):
        ...
        prediction_true = self.compute_model_prediction(self.linear_gaussian_model.true_theta)
        ...
        axs.plot(sorted_xtrain, sorted_prediction.ravel(), label="Prediction", color="r")
        axs.plot(sorted_xtrain, sorted_prediction_true.ravel(), "--", label="True", color="k")
    ...
    def compute_evidence_stats(self, itheta=None):
        ...
        if itheta is None:
            ...
            evidence_cov = self.sub_sampled_vm@self.prior_cov@self.sub_sampled_vm.T + \
