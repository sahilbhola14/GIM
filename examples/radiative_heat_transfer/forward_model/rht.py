...
def setJacobian(jacT, jacbeta, T, T_inf, beta, dy2, relay=False):
    ...
    jacT[ii, ii - 1] = 1.0 / dy2
    jacT[ii, ii] = -2.0 / dy2 - 2e-3 * beta[ii] * T[ii] ** 3
    jacT[ii, ii + 1] = 1.0 / dy2
...
class rht:
    ...
    def __init__(
        self, T_inf=5.0, n_spatial=129, dt=1e-2, n_iter=1000, tol=1e-8, net=None
    ):
        self.n_spatial = n_spatial
        # Temperature of the one-dimensional body
        self.T_inf = T_inf
        self.y = np.linspace(0.0, 1.0, self.n_spatial)  # Coordinates
    ...
    def evalResidual(self):
        ...
        self.res[1:-1] = (
            self.T[0:-2] - 2 * self.T[1:-1] + self.T[2:]
    ...
    def direct_solve(self):
        ...
        for iteration in range(self.n_iter):
            ...
            if res_norm < self.tol:
                ...
                print(
                    "Maximum iterations reached! residual norm : {0:.5f}".format(
                        res_norm
                    )
                )
        return self.res
    ...
    def adjoint_solve(self, data, model_noise_cov, scaling, sample_idx):
        ...
        if model_noise_cov == 0:
            ...
            jac_cost_state = (scaling * (data - prediction) / model_noise_cov).reshape(
                1, -1
            )
