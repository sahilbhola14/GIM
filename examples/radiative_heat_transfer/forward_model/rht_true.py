...
class RHT_True:
    ...
    def __init__(
        ...
        self.T_inf = T_inf
        self.y = np.linspace(0.0, 1.0, npoints)  # Coordinates
    ...
    def getEmiss(self, T):
        ...
        return 1e-4 * (
            self.gamma + self.delta * np.sin(self.alpha * T) + np.exp(0.02 * T)
        )
    ...
    def GaussSeidelUpdate(self, T):
        ...
        T[1:-1:2] = 0.5 * (T[0:-2:2] + T[2::2]) + 0.5 * self.dy2 * (
            emiss[1:-1:2] * (self.T_inf**4 - T[1:-1:2] ** 4)
            + 0.5 * (self.T_inf - T[1:-1:2])
        )

        T[2:-1:2] = 0.5 * (T[1:-2:2] + T[3::2]) + 0.5 * self.dy2 * (
            emiss[2:-1:2] * (self.T_inf**4 - T[2:-1:2] ** 4)
...
def compute_prediction(T_inf, alpha=3 * np.pi / 200, gamma=1, delta=5):
    ...
    rht = RHT_True(
        T_inf=T_inf, n_iter=100000, tol=1e-13, alpha=alpha, gamma=gamma, delta=delta
    )
