...
from compute_identifiability_radiative_heat_transfer import (
...
class inference:
    def __init__(
        ...
        if loaded_ytrain is None:
            ...
            noise = np.sqrt(self.model_noise_cov_scalar) * np.random.randn(
                self.num_data_samples * (self.spatial_res - 2)
            ).reshape(self.num_data_samples, -1)
        ...
        self.model_noise_cov_mat = self.model_noise_cov_scalar * np.eye(
            self.spatial_res
        )
    ...
    def compute_model_prediction(self, theta):
        ...
        for isample in range(self.num_data_samples):
            ...
            T_prediction.append(
                compute_prediction(alpha=alpha, gamma=gamma, delta=delta, T_inf=T_inf)
            )
    ...
    def compute_log_likelihood(self, theta):
        ...
        if self.model_noise_cov_scalar != 0:
            log_likelihood = -0.5 * np.sum(
                error_norm_sq / self.model_noise_cov_scalar, axis=0
            )
        else:
        ...
        self.write_log_file(
            "Scaled Log likelihood : {0:.5e} at theta : {1}".format(
    ...
    def estimate_parameter_conditional_mutual_information(self):
        ...
        estimator.compute_individual_parameter_data_mutual_information_via_mc(
            use_quadrature=True, single_integral_gaussian_quad_pts=60
        )
