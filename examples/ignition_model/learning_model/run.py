...
class learn_ignition_model():
    ...
    def compute_kinetic_parameters(self, theta, initial_temperature, equivalence_ratio):
        ...
        kinetic_parameters = {}
        kinetic_parameters["A"] = self.compute_Arrehenius_A(theta, initial_temperature, equivalence_ratio)
    ...
    def compute_Arrehenius_A(self, theta, initial_temperature, equivalence_ratio):
        ...
        if self.n_parameter_model == 4:
            ...
            l_1 = theta[1]
            l_2 = theta[2]
            log_A = l_0 + np.tanh((l_1 + (l_2*equivalence_ratio))*(initial_temperature/1000))
            ...
            log_A = l_0 + (l_1*np.exp(l_2*equivalence_ratio)) + (l_3*np.tanh(((l_4 + (l_5*equivalence_ratio))*initial_temperature) \
                    + l_6))
    ...
    def compute_model_prediction(self, theta, proc_log_file=None):
        ...
        for idata_sample in range(self.num_data_points):
            ...
            kinetic_parameters = self.compute_kinetic_parameters(theta, initial_temperature, equivalence_ratio)
    ...
    def compute_temperature_prediction(self, theta_samples):
        ...
        for ii in range(num_theta_samples):
            print("Computing prediction for theta sample: {0:d} / {1:d}".format(ii, num_theta_samples), flush=True)
            for idata_sample in range(self.num_data_points):
                initial_pressure = self.initial_pressure[idata_sample]
                initial_temperature = self.initial_temperature[idata_sample]
                equivalence_ratio = self.equivalence_ratio[idata_sample]
                ...
                try:
                    ...
                    interpolated_temp = self.interpolate_state(states.t, states.T, interpolate_time = time_array)
    ...
    def compute_temperature_prediction_2S_Westbrook(self):
        ...
        for idata_sample in range(self.num_data_points):
            ...
            prediction[:, 0, ii] = self.interpolate_state(states.t, states('CH4').X.ravel(), interpolate_time = time_array)
            prediction[:, 1, ii] = self.interpolate_state(states.t, states('CO2').X.ravel(), interpolate_time = time_array)
            prediction[:, 2, ii] = self.interpolate_state(states.t, states('O2').X.ravel(), interpolate_time = time_array)
            prediction[:, 3, ii] = self.interpolate_state(states.t, states('CO').X.ravel(), interpolate_time = time_array)
            prediction[:, 4, ii] = self.interpolate_state(states.t, states('H2O').X.ravel(), interpolate_time = time_array)
            prediction[:, 5, ii] = self.interpolate_state(states.t, states('N2').X.ravel(), interpolate_time = time_array)

    def compute_species_evolution(self, theta_samples, initial_pressure, initial_temperature, equivalence_ratio):
    ...
    def compute_species_evolution_2S_Westbrook(self):
        ...
        for idata in range(self.num_data_points):
            ...
            prediction[:, 0, idata] = self.interpolate_state(states.t, states('CH4').X.ravel(), interpolate_time = time_array)
            prediction[:, 1, idata] = self.interpolate_state(states.t, states('CO2').X.ravel(), interpolate_time = time_array)
            prediction[:, 2, idata] = self.interpolate_state(states.t, states('O2').X.ravel(), interpolate_time = time_array)
            prediction[:, 3, idata] = self.interpolate_state(states.t, states('CO').X.ravel(), interpolate_time = time_array)
            prediction[:, 4, idata] = self.interpolate_state(states.t, states('H2O').X.ravel(), interpolate_time = time_array)
            prediction[:, 5, idata] = self.interpolate_state(states.t, states('N2').X.ravel(), interpolate_time = time_array)
    ...
    def compute_mcmc(self, theta_map, theta_map_cov, num_mcmc_samles):
    ...
    def plot_mcmc_estimate(self, mcmc_samples=None):
        ...
        mcmc_prediction = np.load(os.path.join(self.campaign_path, "mcmc_prediction.npy"))

        # Compute feasibile samples
        feasible_idx = []
        for ii in range(self.num_data_points):
            sub_data = mcmc_prediction[:, ii, :]
            idx = np.arange(sub_data.shape[1])[np.logical_and(np.max(sub_data, axis=0) <1e+4, np.min(sub_data, axis=0) > self.initial_temperature[ii])]
        ...
        for ii in range(self.num_data_points):
            ...
            ub = mean + std
            lb = mean - std
            axs.plot(time_array, true_temperature_prediction[:, ii], "-", color=dark_colors(ii), label="Gri-Mech 3.0, $T_{{o}}$={0:.1f} K".format(self.initial_temperature[ii]))
            axs.plot(time_array, temperature_prediction_2S_Westbrook[:, ii], "-", dashes=[3, 2, 3, 2], lw = 3,  color=dark_colors(ii), label=r"2-step mechanism [Westbrook $\textit{{et al.}}$], $T_{{o}}$={0:.1f} K".format(self.initial_temperature[ii]))
            axs.plot(time_array, mean, "-", dashes=[6, 2, 2, 2], color=dark_colors(ii), lw = 3, label=r"$\mu_{{post}}$, $T_{{o}}$={0:.1f} K".format(self.initial_temperature[ii]))
            axs.fill_between(np.logspace(-6, 0, 3000), ub, lb, ls="-", lw=2,  color=dark_colors(ii), alpha=0.2)
        ...
        fig_save_path = os.path.join(self.campaign_path, "Figures/mcmc_species_prediction.png")

        for idata in range(self.num_data_points):

            axs[0, 0].plot(time_array, species_true_prediction[:, 0, idata], color=dark_colors(idata), label="Gri-Mech 3.0, $T_{{o}}$={0:.1f} K".format(self.initial_temperature[idata]))
            axs[0, 0].plot(time_array, species_2S_Westbrook_prediction[:, 0, idata], "-", dashes=[2, 1, 2, 1], lw = 3, color=dark_colors(idata), label=r"2-step mechanism [Westbrook $\textit{{et al.}}$], $T_{{o}}$={0:.1f} K".format(self.initial_temperature[idata]))
            axs[0, 0].plot(time_array, species_prediction_mean[:, 0, idata], "-", dashes=[6, 2, 2, 2], color=dark_colors(idata), label=r"$\mu_{{post}}$, $T_{{o}}$={0:.1f} K".format(self.initial_temperature[idata]))
            axs[0, 0].fill_between(time_array, species_prediction_mean[:, 0, idata] + species_prediction_std[:, 0, idata], species_prediction_mean[:, 0, idata] - species_prediction_std[:, 0, idata], ls="-", lw=2,  color=dark_colors(idata), alpha=0.2)
            ...
            axs[0, 1].plot(time_array, species_true_prediction[:, 1, idata], color=dark_colors(idata), label="Gri-Mech 3.0")
            axs[0, 1].plot(time_array, species_2S_Westbrook_prediction[:, 1, idata], "-", dashes=[2, 1, 2, 1], lw = 3, color=dark_colors(idata), label=r"$\mu_{{post}}$")
            axs[0, 1].plot(time_array, species_prediction_mean[:, 1, idata], "-", dashes=[6, 2, 2, 2], color=dark_colors(idata), label=r"$\mu_{{post}}$")
            axs[0, 1].fill_between(time_array, species_prediction_mean[:, 1, idata] + species_prediction_std[:, 1, idata], species_prediction_mean[:, 1, idata] - species_prediction_std[:, 1, idata], ls="-", lw=2,  color=dark_colors(idata), alpha=0.2)
            ...
            axs[0, 2].plot(time_array, species_true_prediction[:, 2, idata], color=dark_colors(idata), label="Gri-Mech 3.0")
            axs[0, 2].plot(time_array, species_2S_Westbrook_prediction[:, 2, idata], "-", dashes=[2, 1, 2, 1], lw = 3, color=dark_colors(idata), label=r"$\mu_{{post}}$")
            axs[0, 2].plot(time_array, species_prediction_mean[:, 2, idata], "-", dashes=[6, 2, 2, 2], color=dark_colors(idata), label=r"$\mu_{{post}}$")
            axs[0, 2].fill_between(time_array, species_prediction_mean[:, 2, idata] + species_prediction_std[:, 2, idata], species_prediction_mean[:, 2, idata] - species_prediction_std[:, 2, idata], ls="-", lw=2,  color=dark_colors(idata), alpha=0.2)
            ...
            axs[1, 0].plot(time_array, species_true_prediction[:, 3, idata], color=dark_colors(idata), label="Gri-Mech 3.0")
            axs[1, 0].plot(time_array, species_2S_Westbrook_prediction[:, 3, idata], "-", dashes=[2, 1, 2, 1], lw = 3, color=dark_colors(idata), label=r"$\mu_{{post}}$")
            axs[1, 0].plot(time_array, species_prediction_mean[:, 3, idata], "-", dashes=[6, 2, 2, 2], color=dark_colors(idata), label=r"$\mu_{{post}}$")
            axs[1, 0].fill_between(time_array, species_prediction_mean[:, 3, idata] + species_prediction_std[:, 3, idata], species_prediction_mean[:, 3, idata] - species_prediction_std[:, 3, idata], ls="-", lw=2,  color=dark_colors(idata), alpha=0.2)
            ...
            axs[1, 1].plot(time_array, species_true_prediction[:, 4, idata], color=dark_colors(idata), label="Gri-Mech 3.0")
            axs[1, 1].plot(time_array, species_2S_Westbrook_prediction[:, 4, idata], "-", dashes=[2, 1, 2, 1], lw = 3, color=dark_colors(idata), label=r"$\mu_{{post}}$")
            axs[1, 1].plot(time_array, species_prediction_mean[:, 4, idata], "-", dashes=[6, 2, 2, 2], color=dark_colors(idata), label=r"$\mu_{{post}}$")
            axs[1, 1].fill_between(time_array, species_prediction_mean[:, 4, idata] + species_prediction_std[:, 4, idata], species_prediction_mean[:, 4, idata] - species_prediction_std[:, 4, idata], ls="-", lw=2,  color=dark_colors(idata), alpha=0.2)
            ...
            axs[1, 2].plot(time_array, species_true_prediction[:, 5, idata], color=dark_colors(idata), label="Gri-Mech 3.0")
            axs[1, 2].plot(time_array, species_2S_Westbrook_prediction[:, 5, idata], "-", dashes=[2, 1, 2, 1], lw = 3, color=dark_colors(idata), label=r"$\mu_{{post}}$")
            axs[1, 2].plot(time_array, species_prediction_mean[:, 5, idata], "-", dashes=[6, 2, 2, 2], color=dark_colors(idata), label=r"$\mu_{{post}}$")
            axs[1, 2].fill_between(time_array, species_prediction_mean[:, 5, idata] + species_prediction_std[:, 5, idata], species_prediction_mean[:, 5, idata] - species_prediction_std[:, 5, idata], ls="-", lw=2,  color=dark_colors(idata), alpha=0.2)
    ...
    def plot_mle_estimate(self, theta_mle):
        ...
        for ii,itemp in enumerate(temperature_range):
            ...
            calibrated_model_prediction[ii] = np.log(model_2S_CH4_Westbrook.compute_ignition_time(internal_state="plot"))
        ...
        axs.scatter(1000/self.initial_temperature, self.ytrain.ravel(), label="Data", color="k", marker="s", s=100)
        axs.plot(1000/temperature_range, gri_prediction, "--", label="Gri-Mech 3.0", color="k")
        axs.plot(1000/temperature_range, calibrated_model_prediction, label="M.L.E", color="b")
        axs.plot(1000/temperature_range, un_calibrated_model_prediction, label=r"2-step mechanism [Westbrook \textit{et al.}]", color="r")
        axs.set_xlabel(r"1000/T [1/K]")
        axs.set_ylabel(r"$\log(t_{ign})$")
        axs.legend(loc="upper left")
...
def main():
    ...
    if config_data['compute_mle']:
        ...
        learning_model.compute_mcmc(theta_map, theta_map_cov, num_mcmc_samles=num_mcmc_samples)
