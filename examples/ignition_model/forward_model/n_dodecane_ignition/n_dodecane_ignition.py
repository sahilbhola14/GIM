...
def n_dodecane_combustion(
    equivalence_ratio,
    initial_temp,
    pre_exponential_parameter=27.38,
    activation_energy=31944.0,
    initial_fuel_moles=1,
    campaign_path=None,
    save_plot=False,
):
    ...
    auto_ignition_time, auto_ignition_temp = compute_auto_ignition_time(
        temperature=states.T, time=states.t, dt=dt
    )
    ...
    if save_plot:
        ...
        figure_path = os.path.join(
            campaign_path,
            "true_n_dodecane_combustion_T_{}_phi_{}_rank_{}.png".format(
                initial_temp, equivalence_ratio, rank
            ),
        )
...
def compute_pre_exponential_factor(
    pre_exponential_parameter, equivalence_ratio=0.5, initial_temp=300
):
    ...
    log_pre_exponential_factor = (
        lambda_0
        + (lambda_1 * np.exp(lambda_2 * equivalence_ratio))
        + lambda_3
        * np.tanh(
