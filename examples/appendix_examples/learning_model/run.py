...
class learn_linear_gaussian:
    def __init__(
        ...
        self.model_noise_cov_mat = self.model_noise_cov * np.eye(self.ytrain.shape[1])
    ...
    def compute_model_prediction(self, theta, proc_log_file=None):
        prediction = self.forward_model(theta)[self.sample_idx, :].T
        return prediction
    ...
    def compute_true_posterior(self):
        evidence_mean, evidence_cov = self.compute_evidence_stats()
        correlation = self.prior_cov @ self.sub_sampled_vm.T
        delta = self.ytrain.T - evidence_mean
        mean_solve = np.linalg.solve(evidence_cov, delta)
        post_mean = self.prior_mean + correlation @ mean_solve
        cov_solve = np.linalg.solve(evidence_cov, correlation.T)
        post_cov = self.prior_cov - correlation @ cov_solve

        # cholesky = np.linalg.cholesky(post_cov + 1e-6*np.eye(self.num_parameters))
        # samples = post_mean + cholesky@np.random.randn(self.num_parameters, 1000)
    ...
    def compute_esimated_mi(self):
        ...
        mi_estimator.compute_individual_parameter_data_mutual_information_via_mc(
            use_quadrature=True, single_integral_gaussian_quad_pts=100
        )
...
def main():
    ...
    elif config_data["compute_true_post"]:
        theta_post, theta_post_cov = learning_model.compute_true_posterior()
