...
class unscented_quadrature:
    ...
    def compute_quadrature_points(self):
        ...
        for ipoint in range(1, dim + 1):
            quad_points[:, ipoint] = (
                self.mean.ravel() + np.sqrt(dim + lam) * L[:, ipoint - 1]
            )
            quad_points[:, ipoint + dim] = (
                self.mean.ravel() - np.sqrt(dim + lam) * L[:, ipoint - 1]
            )
    ...
    def compute_integeral(self, importance_sampling_weights=None):
        ...
        def compute_error(x):
            return (x - integral_mean).reshape(-1, 1)
...
class gauss_hermite_quadrature:
    ...
    def compute_quadrature_points_and_weights(self):
        ...
        unrotated_points, weights = self.tensorize_quadrature_points_and_weights(
            quad_points=quad_points, weights=weights
        )
    ...
    def rotate_points(self, unrotated_points):
        ...
        for ipoint in range(rotated_points.shape[1]):
            rotated_points[:, ipoint] = self.mean.ravel() + np.dot(
                L, unrotated_points[:, ipoint]
            )
    ...
    def compute_integeral(self, importance_sampling_weights=None):
        ...
        integral = np.array(
            [integrand[:, ii] * weights[ii] for ii in range(weights.shape[0])]
        )
...
def plot_points(
    num_samples, gaussian_mean, gaussian_cov, integrand_function, view_points=None
):
    """Function plots the points"""
    gaussian_samples = sample_gaussian(
        mean=gaussian_mean, cov=gaussian_cov, num_samples=num_samples
    )
    ...
    axs[0].scatter(gaussian_samples[0, :], gaussian_samples[1, :], color="red", s=5)
    axs[1].scatter(model_samples[0, :], model_samples[1, :], color="red", s=5)
    if view_points is not None:
        axs[0].scatter(view_points[0, :], view_points[1, :], color="k", s=60)
        axs[1].scatter(
            model_evaluated_at_view_points[0, :],
            model_evaluated_at_view_points[1, :],
            color="k",
            s=60,
        )
...
def compute_importance_sampling_weights(eval_points):
    ...
    def target_density_estimator(eval_points):
        proabability = compute_gaussian_prob(
            mean=np.zeros((2, 1)), cov=reference_cov(), samples=eval_points
        )
        return proabability

    def proposal_density_estimator(eval_points):
        proabability = compute_gaussian_prob(
            mean=np.zeros((2, 1)), cov=importance_sampling_cov(), samples=eval_points
        )
        return proabability
...
def main():
    ...
    gh = gauss_hermite_quadrature(
        mean=gaussian_mean, cov=gaussian_cov, integrand=test_function, num_points=3
    )
