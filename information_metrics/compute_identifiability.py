...
class mutual_information:
    def __init__(
        ...
        self.mutual_information_evidence_save_restart_freq = (
            mutual_information_evidence_save_restart_freq
        )
        self.conditional_mutual_information_save_restart_freq = (
            conditional_mutual_information_save_restart_freq
        )
        ...
        self.local_outer_likelihood_prob = np.zeros(self.local_num_outer_samples)
        ...
        if write_proc_log_file:
            proc_log_file_path = os.path.join(
                self.save_path, "proc_{}_log_file.dat".format(rank)
            )
    ...
    def compute_local_num_outer_samples(self):
        ...
        assert (
            self.global_num_outer_samples % size == 0
    ...
    def sample_parameter_distribution(self, num_samples, parameter_pair=None):
        ...
        if parameter_pair is None:
            ...
            (
                selected_parameter_mean,
                selected_parameter_cov,
            ) = self.get_selected_parameter_stats(parameter_pair=parameter_pair)
            parameter_samples = self.sample_gaussian(
    ...
    def sample_prior_distribution(self, num_samples):
        ...
        prior_samples = self.sample_gaussian(
            mean=self.prior_mean, cov=self.prior_cov, num_samples=num_samples
        )
    ...
    def compute_model_prediction(self, theta):
        ...
        for isample in range(num_samples):
            ...
            prediction[:, :, isample] = self.forward_model(
                theta[:, isample], proc_log_file=self.proc_log_file
            )
        return prediction
    ...
    def sample_likelihood(self, theta):
        ...
        noise = np.sqrt(self.model_noise_cov_scalar) * np.random.randn(product).reshape(
    ...
    def evaluate_likelihood_probaility(self, data, model_prediction):
        ...
        likelihood = ((pre_exp) ** (self.num_data_samples)) * np.exp(
            -0.5 * np.sum(error_norm_sq / self.model_noise_cov_scalar, axis=0)
        )

        return likelihood

    def estimate_mutual_information_via_mc(
        self, use_quadrature=False, quadrature_rule="gaussian", num_gaussian_quad_pts=50
    ):
        ...
        if self.outer_data_computation_req_flag is True:
            ...
            (
                self.local_outer_prior_samples,
                is_local_outer_prior_samp_avail,
            ) = self.load_restart_data(data_type="outer", label="outer_prior_samples")
            ...
            (
                self.local_outer_data_samples,
                is_local_outer_data_samp_avail,
            ) = self.load_restart_data(data_type="outer", label="outer_data_samples")
            if is_local_outer_data_samp_avail is False:
                (
                    self.local_outer_data_samples,
                    self.local_outer_model_prediction,
                ) = self.sample_likelihood(theta=self.local_outer_prior_samples)

            # Likelihood probability
            (
                self.local_outer_likelihood_prob,
                is_local_outer_likelihood_prob_avail,
            ) = self.load_restart_data(data_type="outer", label="outer_likelihood_prob")
        ...
        if rank == 0:
            summation = sum(
                [
                    np.sum(global_log_likelihood[ii] - global_log_evidence[ii])
    ...
    def estimate_evidence_probability(
        self, num_gaussian_quad_pts, use_quadrature=False, quadrature_rule="gaussian"
    ):
    ...
    def integrate_likelihood_via_mc(self):
        ...
        (
            local_mutual_information_inner_counter,
            is_mi_inner_counter_avail,
        ) = self.load_restart_data(
            data_type="inner", label="mutual_infomation_inner_counter", sub_type="MI"
        )
        ...
        for isample in range(local_lower_loop_idx, self.local_num_outer_samples):
            ...
            error_norm_sq = np.linalg.norm(error, axis=1) ** 2
            sample_evidence_estimates = pre_exp * np.exp(
                -0.5 * np.sum(error_norm_sq / self.model_noise_cov_scalar, axis=0)
            )
            evidence_prob[isample] = (1 / self.local_num_inner_samples) * np.sum(
                sample_evidence_estimates
            )

            local_mutual_information_inner_counter += 1

            save_condition = (
                isample % self.mutual_information_evidence_save_restart_freq
            )
            final_save_condition = (
        ...
        for isample in range(local_lower_loop_idx, self.local_num_outer_samples):
            ...
            def integrand(eval_theta):
                integrand_val = self.likelihood_integrand(
                    theta=eval_theta, outer_sample=outer_sample
                )
                return integrand_val

            if quadrature_rule == "unscented":
                unscented_quad = unscented_quadrature(
                    mean=self.prior_mean, cov=self.prior_cov, integrand=integrand
                )
    ...
    def create_restart_folders(self):
        ...
        os.mkdir(
            os.path.join(self.restart_file_path, "inner_data", "mutual_information")
        )
        os.mkdir(
            os.path.join(
                self.restart_file_path,
...
class conditional_mutual_information(mutual_information):
    def compute_individual_parameter_data_mutual_information_via_mc(
        ...
        self.write_log_file(
            "Use_quadrature : {} with single intergral quad pts: {}".format(
                use_quadrature, single_integral_gaussian_quad_pts
            )
        )
        ...
        (
            individual_parameter_counter,
            is_individual_parameter_counter_avail,
        ) = self.load_restart_data(
            data_type="inner", label="individual_parameter_counter", sub_type="ICMI"
        )
        ...
        for iparameter in range(lower_parameter_loop_idx, self.num_parameters):
            ...
            if rank == 0:
                summation = sum(
                    [
                        np.sum(
                            global_log_likelihood[ii]
                            - global_log_individual_likelihood[ii]
                        )
                        for ii in range(size)
                    ]
                )
            ...
            self.save_quantity(
                "estimated_individual_mutual_information.npy",
                inidividual_mutual_information,
            )

        self.write_log_file(
            " Normalized MI : {}".format(
                inidividual_mutual_information
    ...
    def compute_posterior_pair_parameter_mutual_information(
        ...
        if os.path.exists("estimated_pair_mutual_information.npy"):
            ...
            pair_mutual_information = np.nan * np.ones(parameter_combinations.shape[0])
        ...
        (
            pair_parameter_counter,
            is_pair_parameter_counter_avail,
        ) = self.load_restart_data(
            data_type="inner", label="pair_parameter_counter", sub_type="PCMI"
        )
        ...
        for jj, parameter_pair in enumerate(
            parameter_combinations[lower_parameter_pair_loop_idx:, :]
        ):

            self.write_log_file(
                ">>> Begin Pair {}/{} computations \n".format(
                    np.arange(parameter_combinations.shape[0])[
                        lower_parameter_pair_loop_idx:
                    ][jj]
                    + 1,
                    parameter_combinations.shape[0],
                )
            )

            (
                individual_likelihood_parameter_counter,
            ...
            (
                ...
                is_local_individual_likelihood_avail,
            ) = self.load_restart_data(
                data_type="inner",
            ...
            if is_individual_likelihood_parameter_counter_avail is False:
                ...
                lower_parameter_loop_idx = (
                    individual_likelihood_parameter_counter.item()
                )

            if is_local_individual_likelihood_avail is False:
                local_individual_likelihood = np.nan * np.ones(
                    (2, self.local_num_outer_samples)
                )
            ...
            for ii, iparameter in enumerate(parameter_pair[lower_parameter_loop_idx:]):
                ...
                local_individual_likelihood[
                    index, :
                ] = self.estimate_individual_likelihood(
                    parameter_pair=np.array([iparameter]),
                    use_quadrature=use_quadrature,
                    num_gaussian_quad_pts=single_integral_gaussian_quad_pts,
                    case_type="pair",
            ...
            global_log_pair_likelihood = comm.gather(local_log_pair_likelihood, root=0)
        ...
        self.write_log_file(
            " Normalized CMI : {}".format(
                pair_mutual_information / np.sum(np.abs(pair_mutual_information))
            )
        )
    ...
    def integrate_individual_likelihood_via_mc(self, parameter_pair):
        ...
        fixed_parameter_idx = self.get_fixed_parameter_id(parameter_pair=parameter_pair)
        ...
        for isample in range(self.local_num_outer_samples):
            ...
            fixed_parameters = self.local_outer_prior_samples[
                fixed_parameter_idx, isample
            ].reshape(-1, 1)
            ...
            sample_individual_likelihood = pre_exp * np.exp(
                -0.5 * np.sum(error_norm_sq / self.model_noise_cov_scalar, axis=0)
            )

            individual_likelihood_prob[isample] = (
                1 / self.local_num_inner_samples
            ) * np.sum(sample_individual_likelihood)

        return individual_likelihood_prob

    def integrate_individual_likelihood_via_quadrature(
        self,
        parameter_pair,
        num_gaussian_quad_pts,
        case_type,
        quadrature_rule="gaussian",
        parent_pair=None,
        importance_sampling_cov_factors=None,
    ):
        ...
        if case_type == "individual":
            likelihood_file_name = "individual_inner_likelihood_parameter_" + str(
                sample_parameter_idx.item()
            )
            counter_file_name = "individual_inner_counter_parameter_" + str(
                sample_parameter_idx.item()
            )
            sub_type = "ICMI"
        elif case_type == "pair":
        ...
        (
            individual_likelihood_prob,
            is_individual_likelihood_prob_avail,
        ) = self.load_restart_data(
            data_type="inner", label=likelihood_file_name, sub_type=sub_type
        )

        (
            local_conditional_mutual_information_counter,
            is_local_conditional_mutual_information_counter_avail,
        ) = self.load_restart_data(
            data_type="inner", label=counter_file_name, sub_type=sub_type
        )
        ...
        for isample in range(local_lower_loop_idx, self.local_num_outer_samples):
            if (
                isample % 1 == 0 or isample == self.local_num_outer_samples - 1
            ) and self.write_proc_log_file:
                self.write_proc_log_file(
                    "   \n(Inner) Sample# : {}/{} [Sampling : {}]\n".format(
                        isample, self.local_num_outer_samples - 1, parameter_pair
                    )
                )
            ...
            if quadrature_rule == "unscented":
                ...
                (
                    individual_likelihood_mean,
                    individual_likelihood_cov,
                ) = unscented_quad.compute_integeral()

                individual_likelihood_prob[
                    isample
                ] = individual_likelihood_mean + np.sqrt(
                    individual_likelihood_cov
                ) * np.random.randn(
                    1
                )
                ...
                if individual_likelihood_prob[isample] == 0:
                    ...
                    for _, ifactor in enumerate(importance_sampling_cov_factors):
                        ...
                        cov_multiplication_factor = ifactor * np.eye(
            ...
            save_condition = (
                isample % self.conditional_mutual_information_save_restart_freq
            )
    ...
    def pair_wise_likelihood_integrand(
        self, theta, fixed_parameters, outer_sample, parameter_pair
    ):
        ...
        error = outer_sample - model_prediction
        error_norm_sq = np.linalg.norm(error, axis=1) ** 2
        sample_evidence_estimates = (pre_exp**self.num_data_samples) * np.exp(
            -0.5 * np.sum(error_norm_sq / self.model_noise_cov_scalar, axis=0)
        )
    ...
    def compute_importance_sampling_weights(
        self, target_density_estimator, proposal_density_estimator, eval_points
    ):
