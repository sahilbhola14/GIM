...
# Date: 14 June 2022
# Description : Computes the model identifiability using Mutual information specific ...
# the radiative heat transfer problem.
...
class mutual_information:
    ...
    def sample_likelihood(self, theta):
        ...
        structured_noise = self.enforce_noise_structure(
            model_prediction=model_prediction, noise=noise
        )
        likelihood_sample = model_prediction + structured_noise
        return likelihood_sample, model_prediction

    def enforce_noise_structure(self, model_prediction, noise):
        """Function enforces noise structure such that the temperature value for the
        RHT problem is non-negative. Futher, no noise is added to the boundary points"""
        structured_noise = noise.copy()
        structured_noise[:, 0, :] = 0
        structured_noise[:, -1, :] = 0
