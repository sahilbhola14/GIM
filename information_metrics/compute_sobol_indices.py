...
class sobol_index:
    def __init__(
        ...
        assert (
            self.local_num_inner_samples % self.inner_batch_size
        ) == 0, "Evenly divide inner batch"
    ...
    def compute_sobol_indices(self):
        ...
        for iparameter in range(self.num_parameters):
            ...
            self.write_log_file(
                "Computing Sobol index for Theta : {} / {}".format(
                    iparameter + 1, self.num_parameters
                )
            )
            self.write_log_file(
                ">>>-----------------------------------------------------------<<<"
            ...
            (
                outer_selected_parameter_samples,
                is_outer_selected_parameter_samples_avail,
            ...
            if is_outer_selected_parameter_samples_avail is False:
                ...
                selected_mean, selected_cov = self.get_selected_parameter_stats(
            ...
            local_reshaped_inner_expectation_samples = (
                local_inner_expectation_samples.reshape(
                    -1, self.local_num_outer_samples
                )
            )
            local_batch_unnormalized_mean = (
                self.local_num_outer_samples / self.global_num_outer_samples
            ) * np.mean(local_reshaped_inner_expectation_samples)
            global_batch_unnormalized_mean_list = comm.gather(
                local_batch_unnormalized_mean, root=0
            )
            if rank == 0:
            ...
            local_inner_expectation_samples_mean_removed_sq = (
                local_inner_expectation_samples - local_mean
            ) ** 2
            # Reshaping (Assuming data samples are i.i.d.)
            local_inner_expectation_samples_mean_removed_sq_reshaped = (
                local_inner_expectation_samples_mean_removed_sq.reshape(
                    -1, self.local_num_outer_samples
                )
            )
            local_batch_unnormalized_var = (
                self.local_num_outer_samples / self.global_num_outer_samples
            ) * np.mean(local_inner_expectation_samples_mean_removed_sq_reshaped)
            global_batch_unnormalized_var_list = comm.gather(
                local_batch_unnormalized_var, root=0
            )
            if rank == 0:
    ...
    def compute_inner_expectation(
        self, outer_selected_parameter_samples, parameter_pair
    ):
        """Function computes the inner expectation"""
        # local inner files
        inner_expectation_counter_file_name = (
            "inner_expectation_counter_parameter_{}".format(parameter_pair[0])
        )
        inner_expectation_samples_file_name = (
            "inner_expectation_samples_parameter_{}".format(parameter_pair[0])
        )

        # Load data
        (
            local_inner_expectation_samples,
            is_local_inner_expectation_samples_avail,
        ) = self.load_restart_data(
            data_type="inner", label=inner_expectation_samples_file_name
        )

        # Load counter
        (
            local_inner_expectation_counter,
            is_local_inner_expectation_counter_avail,
        ) = self.load_restart_data(
            data_type="inner", label=inner_expectation_counter_file_name
        )
        if is_local_inner_expectation_samples_avail is False:
            local_inner_expectation_samples = np.nan * np.ones(
        ...
        if is_local_inner_expectation_counter_avail is False:
            ...
            self.write_log_file(
                "Inner expectation restarted from : {}/{}".format(
                    local_lower_loop_idx, self.local_num_outer_samples
                )
            )

        # Pending evaluations
        num_pending_inner_evaluations = (
            self.local_num_outer_samples - local_inner_expectation_counter
        )

        # Inner sample parameter
        inner_sample_parameter = self.get_fixed_parameter_id(
            parameter_pair=parameter_pair
        )

        # Selected parameter stats
        selected_mean, selected_cov = self.get_selected_parameter_stats(
            parameter_pair=inner_sample_parameter
        )
        ...
        for iouter in range(local_lower_loop_idx, self.local_num_outer_samples):
            if (iouter % 50 == 0) or (iouter == self.local_num_outer_samples - 1):
                self.write_log_file(
                    "     (Inner expectation) Outer sample #{}/{}".format(
                        iouter + 1, self.local_num_outer_samples
                    )
                )

            # Build eval samples
            eval_parameter[parameter_pair, :] = outer_selected_parameter_samples[
                0, iouter
            ]

            num_inner_batches = int(
                self.local_num_inner_samples / self.inner_batch_size
            )
            ...
            for ibatch in range(num_inner_batches):
                inner_batch_samples = eval_parameter[
                    :,
                    ibatch
                    * self.inner_batch_size : (ibatch + 1)
                    * self.inner_batch_size,
                ]
                inner_batch_prediction = self.compute_model_prediction(
                    theta=inner_batch_samples
                )
                inner_batch_expectation = np.mean(inner_batch_prediction, axis=-1)
                inner_expectation += (
                    self.inner_batch_size / self.local_num_inner_samples
                ) * inner_batch_expectation
